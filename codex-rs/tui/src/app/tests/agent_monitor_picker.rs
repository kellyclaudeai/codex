...
async fn monitor_picker_updates_tokens_and_completion_without_reopening() -> Result<()> {
    ...
    app.agent_navigation.upsert(worker, Some("Worker".to_string()), None, false);
    app.agent_navigation.monitor.observe(worker, &turn_started_notification(worker, "one"));
    ...
    app.agent_navigation.monitor.observe(worker, &token_usage_notification(worker, "one", None));
    ...
    app.agent_navigation.monitor.observe(worker, &turn_completed_notification(worker, "one", TurnStatus::Completed));
    app.repaint_agent_picker(selected);
    ...
    let params = app.agent_picker_selection_view_params(Some(1));
    app.chat_widget.replace_selection_view_if_present(super::super::agent_picker::AGENT_PICKER_VIEW_ID, params);
...
async fn monitor_picker_keeps_failed_agents_visible_and_fits_narrow_terminal() -> Result<()> {
    ...
    app.agent_navigation.upsert(worker, Some("Worker".to_string()), None, false);
    app.agent_navigation.monitor.observe(worker, &turn_started_notification(worker, "one"));
    app.agent_navigation.monitor.observe(worker, &turn_completed_notification(worker, "one", TurnStatus::Failed));
